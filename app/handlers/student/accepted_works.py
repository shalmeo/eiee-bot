...
@router.callback_query(
    ProfileCallbackFactory.filter(F.action == ProfileAction.ACCEPTED)
)
async def on_accepted_home_works(call: types.CallbackQuery, student_repo: StudentRepo):
    home_works = await student_repo.get_accepted_home_works()

    text = [
        "Принятые работы (последние 5)\n\n",
